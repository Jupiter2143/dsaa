...
public class GUI {
    ...
    private boolean isSelectionToolActive = false; // 新增变量，控制套索工具是否激活
        ...
        label.addMouseListener(
                new MouseAdapter() {
                    ...
                    public void mousePressed(MouseEvent e) {
                        if (isSelectionToolActive) { // 检查套索工具是否激活
...
//                });
        selectButton.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        isSelectionToolActive = !isSelectionToolActive; // 切换套索工具的状态
                        if (isSelectionToolActive) {
                            startSelectionTool(); // 如果需要，可以添加一些开始套索工具的逻辑
                        } else {
                            // 如果需要，可以添加一些结束套索工具的逻辑
                            highPriorityPoints.clear();
                            lowPriorityPoints.clear();
                            label.repaint();
                        }
    ...
    private void startSelectionTool() {
        ...
        JOptionPane.showMessageDialog(
                frame, "请在图像上用鼠标拖动进行选区。画出保护区域请用鼠标左键（显示为红色），画出消除区域请用鼠标右键（显示为绿色）。", "套索工具", JOptionPane.INFORMATION_MESSAGE);
    }
